...
class T1_Omni(BaseTask):
    ...
    def _resample_commands(self):
        ...
        if self.cfg["commands"]["curriculum"]:
            ...
            scaled_commands = commands_vec * commands_scale

            # Normalize to unit sphere
            norm = torch.norm(scaled_commands, dim=1)
            exceeds_one = norm > 1.0
            scaled_commands[exceeds_one] = scaled_commands[exceeds_one] / norm[exceeds_one].unsqueeze(1)

            self.commands[env_ids, :3] = scaled_commands
    ...
    def _resample_curriculum_commands(self, env_ids):
        ...
        self.commands[env_ids, 3] = (
            torch_rand_float(self.cfg["commands"]["ball_vel"][0], self.cfg["commands"]["ball_vel"][1], (len(env_ids), 1), device=self.device).squeeze(
                1
            )
            * self.cfg["normalization"]["ball_vel"]
        )
    ...
    def _compute_observations(self):
        ...
        self.obs_buf = torch.cat(
            (
                ...
                self.commands[:, :8],  # x, y, yaw (rad/s), placeholder1, placeholder2
        ...
        # Ensure obs length matches configured num_obs; pad/truncate with explicit debug once
        if self.obs_buf.shape[1] != self.num_obs:
            diff = self.num_obs - self.obs_buf.shape[1]
            if not hasattr(self, "_printed_obs_mismatch") or not self._printed_obs_mismatch:
                print(f"Obs length mismatch: got {self.obs_buf.shape[1]}, expected {self.num_obs}. diff={diff}")
                self._printed_obs_mismatch = True
            if diff > 0:
                pad = torch.zeros(self.num_envs, diff, device=self.device, dtype=self.obs_buf.dtype)
    ...
    def _compute_asymmetric_vel_error(self, command_idx, current_vel, vel_type):
        """Computes asymmetric velocity tracking error."""
        # vel_type should be 'lin_vel' or 'ang_vel'
        scaling_factor = self.cfg["normalization"][vel_type]
        target_vel = self.commands[:, command_idx] / scaling_factor
        error = target_vel - current_vel

        underspeed_scale = self.cfg["rewards"].get("underspeed_penalty_scale", 1.0)
        
        # Condition for underspeed: error has the same sign as target velocity.
        is_underspeed = (torch.sign(error) * torch.sign(target_vel)) > 0.
        
        penalty_scale = torch.where(is_underspeed, underspeed_scale, 1.0)
        
        # Apply penalty to the squared error
        loss = penalty_scale * torch.square(error)
        
        return torch.exp(-loss / self.cfg["rewards"]["tracking_sigma"])

    def _reward_tracking_lin_vel_x(self):
        # Tracking of linear velocity commands (x axes)
        return self._compute_asymmetric_vel_error(0, self.filtered_lin_vel[:, 0], "lin_vel")

    def _reward_tracking_lin_vel_y(self):
        # Tracking of linear velocity commands (y axes)
        return self._compute_asymmetric_vel_error(1, self.filtered_lin_vel[:, 1], "lin_vel")

    def _reward_tracking_ang_vel(self):
        # Tracking of angular velocity commands (yaw)
        return self._compute_asymmetric_vel_error(2, self.filtered_ang_vel[:, 2], "ang_vel")
    ...
    def _reward_feet_swing(self):
        ...
        swing_reward = (left_swing & ~self.feet_contact[:, 0]).float() + (right_swing & ~self.feet_contact[:, 1]).float()
        both_feet = (self.feet_contact[:, 0] & self.feet_contact[:, 1]).float()
        
        # Reward matching the walking gait or being in a flight phase for running, and penalize stumbling
        return swing_reward - both_feet
